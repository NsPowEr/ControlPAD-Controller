    ...
    with open(pcapng_file, 'rb') as f:
        ...
        for ts, buf in reader:
            ...
            if direction == 0 and data_length == 64:
                ...
                # skip known
                if payload[0] == 0x51 and payload[1] in (0x20, 0x18, 0x80, 0x94, 0x28, 0x90): continue
                if payload[0] == 0x56: continue
                if payload[0] == 0x55: continue
                if payload[0] == 0x41: continue
                if payload[0] == 0x52: continue
                if payload[0] == 0x00: continue
                print(binascii.hexlify(payload[:16]).decode('utf-8'))
